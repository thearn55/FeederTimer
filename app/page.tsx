...
type Active = { startedAt:number; currentSide:Side; segmentStartedAt:number; leftDuration:number; rightDuration:number; isPaused:boolean } | null;
...
export default function Home() {
  ...
  function chooseSide(side:Side) {
    ...
    if(!active) { setActive({startedAt:timestamp,currentSide:side,segmentStartedAt:timestamp,leftDuration:0,rightDuration:0,isPaused:false}); return; }
  ...
  function pauseOrResume() {
    ...
    if(active.isPaused) { setActive({...active,isPaused:false,segmentStartedAt:timestamp}); return; }
    const segment=Math.max(0,Math.floor((timestamp-active.segmentStartedAt)/1000));
    setActive({...active,isPaused:true,segmentStartedAt:timestamp,leftDuration:active.leftDuration+(active.currentSide==='left'?segment:0),rightDuration:active.rightDuration+(active.currentSide==='right'?segment:0)});
  }
  function finish() {
  ...
  async function refreshHistory() {
    ...
    try { const cloud=await refreshHouseholdEntries(household); setEntries(cloud as FeedEntry[]); setSyncState('synced'); }
